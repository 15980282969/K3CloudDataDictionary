...
namespace K3CloudDataDictionary
{
    public partial class MainWindow
    {
        ...
        private async void MainWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (vm?.CurrentConnection != null)
            {
                // 迁移旧的 metadata.db 到按连接命名的新文件（仅旧版升级场景）
                SQLiteHelper.MigrateOldMetadataDb(vm.CurrentConnection);

                // 根据当前连接计算本地数据路径
                vm.UpdateLocalDbPath();

                if (vm.HasLocalData)
                {
                    // 确认该文件是自动生成的（关联此连接），而非导入的文件恰好同名
                    var localFiles = SQLiteHelper.ScanLocalDataFiles();
                    var expectedFileName = vm.CurrentConnection.EffectiveLocalDbFileName;
                    var autoFile = localFiles.FirstOrDefault(f =>
                        f.FileName.Equals(expectedFileName, StringComparison.OrdinalIgnoreCase) && f.IsAutoGenerated);

                    if (autoFile != null)
                    {
                        vm.IsConnected = true;
                        vm.StatusText = $"已连接：{vm.CurrentConnection.DisplayName}（本地数据）";
                        await vm.LoadTreeDataAsync();
                    }
                    else if (!string.IsNullOrWhiteSpace(vm.CurrentConnection.ServerIp))
                    {
                        vm.IsConnected = true;
        ...
        {
            var vm = DataContext as MainViewModel;
            var dialog = new ConnectionDialog(vm?.CurrentConnection, vm?.LocalDbPath);
            dialog.Owner = this;
            ...
            if (dialog.ShowDialog() == true)
            {
                ...
                if (!string.IsNullOrEmpty(dialog.SelectedLocalDbPath))
                {
                    // 从本地数据页签连接：直接使用指定的本地数据文件
                    vm.LocalDbPath = dialog.SelectedLocalDbPath;
                    vm.CurrentConnection = dialog.SelectedConnection;
                    vm.IsConnected = true;
                    vm.OpenTabs.Clear();

                    if (vm.HasLocalData)
                    {
                        vm.StatusText = dialog.SelectedConnection != null
                            ? $"已连接：{dialog.SelectedConnection.DisplayName}（本地数据）"
                            : "本地数据模式";
                        await vm.LoadTreeDataAsync();
                    }
                    else
                    {
                        vm.StatusText = "本地数据文件不存在";
                    }
                }
                else if (dialog.SelectedConnection != null)
                {
            ...
            // 对话框关闭后，检查当前使用的本地数据文件是否已被删除
            CheckLocalDataAvailability();
        }

        /// <summary>
        /// 检查当前本地数据文件是否仍然存在，不存在则清空主界面数据
        /// </summary>
        private void CheckLocalDataAvailability()
        {
            ...
            if (!string.IsNullOrEmpty(vm.LocalDbPath) && !System.IO.File.Exists(vm.LocalDbPath))
            {
                vm.LocalDbPath = null;
                vm.ModuleTree.Clear();
                vm.OpenTabs.Clear();
                vm.SelectedTab = null;
                vm.IsConnected = false;
                vm.StatusText = "本地数据已删除 | 请重新连接或导入数据";
            }
        }
        ...
        private void RefreshMetadata_Click(object sender, RoutedEventArgs e)
        {
            ...
            var localDbPath = SQLiteHelper.GetLocalDbPath(vm.CurrentConnection);
        ...
        private async void FieldDataGrid_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                await vm.OpenAssistantDataTabAsync(field.LookUpObjectID, field.LookUpObjectDisplay);
            }
            else if (!string.IsNullOrWhiteSpace(field.LookUpObjectID))
            {
        ...
        private async void AllFieldsDataGrid_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                await vm.OpenAssistantDataTabAsync(allField.LookUpObjectID, allField.LookUpObjectDisplay);
            }
            else if (!string.IsNullOrWhiteSpace(allField.LookUpObjectID))
            {
